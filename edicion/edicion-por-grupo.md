...
El desplegable de campo comienza con la opción **Sin filtro** (_No Filter_). Mientras se mantiene esa selección, los controles de operador y valor permanecen ocultos y la acción afecta a todos los elementos del tipo elegido.
...
**Cambiar material a tuberías de un diámetro concreto** Elemento: Pipes — Filtro: `Diameter = 200` — Do: `Material → Replace with → PVC`

**Incrementar el 10 % la rugosidad de tuberías de fundición** Filtro: `Material = FD` — Do: `Roughness → Multiply by → 1.1`

**Cerrar todas las válvulas de aislamiento** Elemento: Isolation Valves — Filtro: Sin filtro — Do: `InitStatus → Replace with → CLOSED`

**Asignar patrón a un conjunto de nudos seleccionados** Marcar "Only selected features" — Elemento: Junctions — Do: `Pattern → Replace with → PAT_RESIDENCIAL`

**Reemplazar texto en etiquetas** Elemento: Junctions — Do: `Tag → Find and replace → Buscar: "SEC" / Reemplazar: "ZN"`
