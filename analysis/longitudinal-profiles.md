...
* The right Y axis has its own scale and label settings, accessible in **Chart options → Axes** (see [Chart customization](longitudinal-profiles.md#personalización-del-gráfico)).
...
| Button                                   | Function                                                                                                                                                                                                                                                                                        |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Edit paths** (pencil icon, toggleable) | Activate editing mode: left click to trace the route node by node, right click on a node to see its options (see [Mouse shortcuts](longitudinal-profiles.md#atajos-de-ratón)). When disabled, moving the mouse over the path only highlights it and displays information, without modifying it. |
| **Help** (ⓘ icon)                        | Opens the **"How to edit paths"** dialog, with a summary of all available editing actions and mouse shortcuts.                                                                                                                                                                                  |

> 📝 Adding an intermediate step node, deleting it, moving it or creating a branch no longer has its own button on the toolbar: they are done with **Edit paths** active, using the context menu (right click) or the mouse shortcuts described in [Mouse shortcuts](longitudinal-profiles.md#atajos-de-ratón). These actions work the same on the main route and on the branches.
...
| **Envelope**      | Opens a submenu to activate the Min/Max envelope of the simulation (see section [Envelope](longitudinal-profiles.md#envolvente-minmax)) |
...
| **Export configuration** | Save the current profile settings to a file `.cfg` (see section [Import and export settings](longitudinal-profiles.md#importar-y-exportar-configuración)) |
...
**Axes Tab** For each axis (X = distance, Y = variable):
...
**Curves Tab** For each active series:
