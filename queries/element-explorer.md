...
> ⚠️ **Conditional quality fields.** The `Quality` field only appears when the project quality model is not _None_. The `ReactRate` field is only visible when the quality model is _Chemical_; remains hidden for _None_, _Age_ and _Trace_ models. These fields are only displayed when the project's quality model supports them.
