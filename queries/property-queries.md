...
> ⚠️ **Conditional quality fields.** The result fields `Quality` and `ReactRate` only appear when the project's quality model allows it: `Quality` is hidden with _None_ model and `ReactRate` is only visible with _Chemical_ model. Static quality fields (`BulkCoeff`, `WallCoeff`, `ReactCoef`, `IniQuality`) are hidden when the quality model is _None_, _Age_, or _Trace_.
